...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DEL] = "images\\Menu_Delete.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_DICE_VALUE] = "images\\Menu_Dice_Value.jpg";
...
void Output::PrintPlayersInfo(string info)
{
	...
	ClearToolBar(); //make a problem in printing so wt
...
///TODO: Set pen color and width using the appropriate variables of UI_Info object (UI) again?!!!
	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
	...
	int numCrossLines = (fromStartY - toStartY) / UI.CellHeight;
	for (int i = 1; i <= numCrossLines; ++i)
	{
		int crossY = y1 - ((i - .5) * UI.CellHeight);
		pWind->DrawLine(x12, crossY, x34, crossY);
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int yTop = y1 - yChange;
	int yBottom = y1 + yChange;

	int pointsX[] = { x12, xRight, x12, xLeft };
	int pointsY[] = { yTop, y1, yBottom, y1 };


	///TODO: Draw the Polygon (diamond) representing the Snake's Head#
	//       Check the snakes drawn in the project document and draw it the same way

	pWind->DrawPolygon(pointsX, pointsY, 4);
